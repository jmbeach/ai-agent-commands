...
### `codereview-gh` [🔗](./commands/codereview-gh.md)

Yet another way to perform a code review. This one uses a GitHub PR to detect the changes. It is faster than the base `/codereview` command, but still tends to miss things that the `/codereview` command catches.
...
### `/address-pr-comments` [🔗](./commands/address-pr-comments.md)
